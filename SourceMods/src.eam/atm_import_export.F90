...
! *** Add water tracer source definition here:
                     ...
                     if(j .eq. 2) then
                        !ARC: Arctic water vapour
                        if((wtlat >= 70.0_r8) .and. (wtlat <= 90.0_r8)) then
                           cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) = -x2a(index_x2a_Faxx_evap,ig)
                        else
                           cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) = 0._r8
                        end if
                     else if(j .eq. 3) then
                        !ROW: Bulk water copy for ratio calculation. Required in each water tracer run.
                        if((wtlat >= -90.0_r8) .and. (wtlat <= 70.0_r8)) then
                           cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) = -x2a(index_x2a_Faxx_evap,ig)
                        else
                           cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) = 0._r8
                        end if
